...
namespace TarkovAutoShade
{
    internal static class ImageAnalyzer
    {
        ...
        private const int MinimumScreenshotHeight = 360;
        ...
        public static AnalysisResult Analyze(string filePath, AppSettings settings)
        {
            ...
            {
                if (source.Width < MinimumScreenshotWidth || source.Height < MinimumScreenshotHeight)
                {
                    return new AnalysisResult {
                        ...
                        IsUsable = false,
                        SkipReason = $"截图分辨率过低（{source.Width}×{source.Height}）"
                    };
                }
        ...
        {
            if (result.P99 <= 3.0 / 255.0)
            {
                result.IsUsable = false;
                result.SkipReason = "截图接近全黑，可能处于加载或过渡画面";
                result.SceneLabel = "无效画面";
                return;
            }
            if (result.P01 >= 252.0 / 255.0)
            {
                result.IsUsable = false;
                result.SkipReason = "截图接近全白，可能处于过渡画面";
                result.SceneLabel = "无效画面";
                return;
            }
            if (result.DynamicRange <= 4.0 / 255.0 && result.EdgeEnergy <= 0.0025)
            {
                result.IsUsable = false;
                result.SkipReason = "截图几乎没有可分析的明暗细节";
                result.SceneLabel = "无效画面";
                return;
            }
