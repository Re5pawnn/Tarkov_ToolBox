...
_TRACKER_BACKGROUND_REFRESH_LOCK = threading.Lock()
_TRACKER_BACKGROUND_REFRESHING = False
NAME_MAX_LENGTH = 24
...
def is_tracker_cache_stale(cache: dict[str, Any] | None) -> bool:
    timestamp = cache.get("updatedAt") if isinstance(cache, dict) else None
    return not isinstance(timestamp, (int, float)) or time.time() - float(timestamp) > TRACKER_CACHE_TTL_SEC


def start_tracker_cache_refresh(reason: str) -> None:
    global _TRACKER_BACKGROUND_REFRESHING

    with _TRACKER_BACKGROUND_REFRESH_LOCK:
        if _TRACKER_BACKGROUND_REFRESHING:
            return
        _TRACKER_BACKGROUND_REFRESHING = True

    def worker() -> None:
        global _TRACKER_BACKGROUND_REFRESHING
        try:
            refresh_tracker_cache()
            append_startup_log(f"Tracker cache refreshed in background: {reason}")
        except Exception as exc:  # noqa: BLE001
            append_startup_log(f"Tracker cache background refresh failed ({reason}): {exc}")
        finally:
            with _TRACKER_BACKGROUND_REFRESH_LOCK:
                _TRACKER_BACKGROUND_REFRESHING = False

    threading.Thread(target=worker, name=f"tracker-cache-refresh-{reason}", daemon=True).start()


def ensure_tracker_cache(force: bool = False) -> tuple[dict[str, Any] | None, str | None]:
    cache = read_tracker_cache()
    if cache and not force:
        if is_tracker_cache_stale(cache):
            start_tracker_cache_refresh("stale")
        return cache, None
...
def run() -> int:
    ...
    thread.start()
    if is_tracker_cache_stale(read_tracker_cache()):
        start_tracker_cache_refresh("startup")
