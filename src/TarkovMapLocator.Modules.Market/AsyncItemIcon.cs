...
public sealed class AsyncItemIcon : Border
{
    ...
    public static readonly DependencyProperty ItemIdProperty = DependencyProperty.Register(
        nameof(ItemId), typeof(string), typeof(AsyncItemIcon), new PropertyMetadata("", OnInputChanged));

    public static readonly DependencyProperty IconLinkProperty = DependencyProperty.Register(
        nameof(IconLink), typeof(string), typeof(AsyncItemIcon), new PropertyMetadata("", OnInputChanged));

    public static readonly DependencyProperty FallbackIconLinkProperty = DependencyProperty.Register(
        nameof(FallbackIconLink), typeof(string), typeof(AsyncItemIcon), new PropertyMetadata("", OnInputChanged));

    public static readonly DependencyProperty PreferFullImageProperty = DependencyProperty.Register(
        nameof(PreferFullImage), typeof(bool), typeof(AsyncItemIcon), new PropertyMetadata(false, OnInputChanged));
    ...
    {
        if (source is AsyncItemIcon icon) icon.QueueLoad();
    }
    ...
    private async void LoadAsync()
    {
        ...
        try
        {
            var source = await host.LoadItemIconAsync(ItemId, IconLink, FallbackIconLink, PreferFullImage);
        ...
        {
            // Artwork is decorative; keep the placeholder after transient failures.
        }
    }
}
